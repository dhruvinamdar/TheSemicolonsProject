...
function populateHiddenField(id){
	...
	console.log("Populate Hidden Field" + id);
...
function showListOfOrders(event){
	...
	xhr.onreadystatechange=function(){
		if(xhr.readyState===4 && xhr.status==200){
			var customerQuoteData = xhr.responseText;
			console.log("SHOWLIST KA" + customerQuoteData);
...
function showApprovedOrder(customerQuoteData){
	...
	console.log("IN SHOW APPLROVED SCRIPT" + customerQuoteData);
	...
	console.log(customerQuoteData.length);
	console.log("Ssfdfdd" + customerQuoteData);
    for (var c = 0; c <= customerQuoteData.length - 1; c++) {
            ...
            td = tr.insertCell(-1);
			var orderId = customerQuoteData[c].orderId;
			console.log("SHOW APPROVED ORDER ORDER ID" + orderId);
            td.innerHTML = customerQuoteData[c].orderId;                 // ADD VALUES TO EACH CELL.
            td = tr.insertCell(-1);
            td.innerHTML = customerQuoteData[c].orderDate; 
            ...
            td = tr.insertCell(-1);
			td.innerHTML = '<button type="submit" class="btn btn-primary" id="'+orderId+'"  onclick="populateHiddenField(orderId)">Show Invoice</button>';
